...
@section('title', 'Quản lý Công việc IT')

@section('content')
<div class="dashboard-content">
    <div class="page-header">
        <h2 class="page-title">Quản lý Công việc IT</h2>
        <a href="{{ route('admin.it-jobs.create') }}" class="btn btn-success">
    ...
    <div class="content-card">
        ...
        <form action="{{ route('admin.it-jobs.index') }}" method="GET" class="search-form">
            <div class="form-group">
                ...
                <input type="text" 
                       ...
                       placeholder="Nhập tên công việc hoặc mô tả..."
                       class="form-input">
            </div>
            <div class="form-group">
        ...
        <div class="table-responsive">
            <table class="admin-table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            <input type="checkbox" class="row-checkbox" value="{{ $job->job_id }}" onchange="updateBulkActions()">
                        </td>
                        ...
                        <td>
                            @if($job->category)
                            <span class="category-badge">{{ $job->category->category_name }}</span>
                            @else
                            <span class="text-muted">-</span>
                            @endif
                        </td>
                        <td>{{ Str::limit($job->short_description ?? '-', 50) }}</td>
                        <td>{{ $job->created_at->format('d/m/Y') }}</td>
                        <td class="action-column">
                            <a href="{{ route('admin.it-jobs.edit', $job->job_id) }}" class="btn-edit">
                                <i class="bi bi-pencil"></i> Sửa
                            </a>
                            <form action="{{ route('admin.it-jobs.destroy', $job->job_id) }}" 
                                  method="POST" 
                                  class="d-inline"
        ...
        <div class="pagination-info">
            @if($jobs->total() > 0)
                Hiển thị {{ $jobs->firstItem() }} đến {{ $jobs->lastItem() }} trong tổng số {{ $jobs->total() }} bản ghi
            @else
...
<!-- Bulk Delete Form -->
<form id="bulkDeleteForm" action="{{ route('admin.it-jobs.destroy-multiple') }}" method="POST" style="display: none;">
    @csrf
    @method('POST')
